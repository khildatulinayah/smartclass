...
<div class="mb-8 max-w-6xl mx-auto">
    ...
    <div class="pixel-card p-4 bg-white shadow mb-8">
        ...
        <form action="{{ route('bendahara.kas.store') }}" method="POST" class="grid gap-4 md:grid-cols-2">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}" class="pixel-card w-full p-3 border-2 border-gray-300" required>
                @error('date')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="pixel-font text-xs text-gray-700">Jumlah (Rp)</label>
                <input type="number" name="amount" min="1" step="0.01" value="{{ old('amount') }}" class="pixel-card w-full p-3 border-2 border-gray-300" required>
                @error('amount')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="pixel-font text-xs text-gray-700">Deskripsi</label>
                <input type="text" name="description" value="{{ old('description') }}" class="pixel-card w-full p-3 border-2 border-gray-300" required>
        ...
        <div class="pixel-card p-4 bg-green-100 overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="pixel-card px-3 py-2 bg-white text-xs">{{ $income->date->format('d/m/Y') }}</td>
                        <td class="pixel-card px-3 py-2 bg-white text-xs">{{ $income->description }}</td>
                        <td class="pixel-card px-3 py-2 bg-white text-right text-xs font-bold">Rp {{ number_format($income->amount, 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="pixel-card p-4 bg-red-100 overflow-x-auto">
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="pixel-card px-3 py-2 bg-white text-xs">{{ $expense->date->format('d/m/Y') }}</td>
                        <td class="pixel-card px-3 py-2 bg-white text-xs">{{ $expense->description }}</td>
                        <td class="pixel-card px-3 py-2 bg-white text-right text-xs font-bold">Rp {{ number_format($expense->amount, 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="pixel-card p-6 bg-yellow-200 text-center">
        ...
        <p class="pixel-font text-3xl text-yellow-600">Rp {{ number_format($balance, 0, ',', '.') }}</p>
    </div>
</div>
@endsection