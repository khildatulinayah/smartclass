...
<div class="w-full px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('sekretaris.dashboard') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition">
            ← Dashboard
        </a>
    </div>
    ...
    <div class="flex items-center justify-between mb-6 bg-white p-4 rounded-lg shadow-sm border">
        <div class="flex items-center space-x-2">
            <a href="{{ route('sekretaris.absensi', ['date' => \Carbon\Carbon::parse($selectedDate)->subDay()->format('Y-m-d')]) }}" 
            ...
            <a href="{{ route('sekretaris.absensi', ['date' => \Carbon\Carbon::parse($selectedDate)->addDay()->format('Y-m-d')]) }}" 
               class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition">
                Besok →
            </a>
        </div>
        <p class="text-lg font-semibold text-gray-800">
            {{ \Carbon\Carbon::parse($selectedDate)->locale('id')->format('l, d F Y') }}
        </p>
    </div>
        ...
        <div class="bg-red-100 border-2 border-red-400 text-red-800 px-6 py-4 rounded-lg mb-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg">{{ $holiday->note }}</p>
                    <p class="text-sm mt-1 opacity-75">Dibuat oleh: {{ $holiday->creator->name ?? 'Sistem' }}</p>
                </div>
                <form action="{{ route('sekretaris.absensi.delete_holiday') }}" method="POST" class="inline">
                    @csrf
